            ...
            if message.get("role") == "user":
                for content in message.get("content", []):
                    if "toolResult" in content:
                        ...
                        logger.info(f"Tool result - ID: {tool_result.get('toolUseId')}, Status: {tool_result.get('status')}")
                        # ツール実行結果の内容も記録
                ...
                for content in message.get("content", []):
                    if "toolUse" in content:
                        ...
                        logger.info(f"Tool executed: {tool_use.get('name')} with args: {tool_use.get('input', {})}")
